...
class Runner:
    def __init__(self, args):
        ...
        self.lr_conf = self.conf["train.lr_conf"]
        ...
        self.model = SuRF(self.conf["model"]).to(self.device)
        ...
        optim_param = self.model.get_optim_params(lr_conf=self.lr_conf)
        ...
        # self.lr_scheduler = CosineAnnealingWarmRestarts(self.optimizer, T_0=4, T_mult=2)
    ...
    def train_epoch(self, epoch):
        ...
        if is_main_process():
            pwidgets = [progressbar.Percentage(), " ", progressbar.Counter(format='%(value)02d/%(max_value)d'), " ", progressbar.Bar(), " ",
                ...
                progressbar.Variable('dl', width=1), ",", progressbar.Variable('adl', width=1), ",", 
                progressbar.Variable('pl', width=1), ",", progressbar.Variable('psnr', width=1)]
        ...
        for batch, inputs in enumerate(self.train_loader):
            ...
            if is_main_process():
                pbar.update(batch, LR=self.optimizer.param_groups[0]['lr'], 
                            ...
                            adl="{:.3f}|{:.3f}".format(scalar_opts["auxi_depth_loss"], avg_scalars.avg_data["auxi_depth_loss"]),
                            pl="{:.3f}|{:.3f}".format(scalar_opts["photo_loss"], avg_scalars.avg_data["photo_loss"]),
    ...
    def validate(self, epoch=0):
        ...
        if is_main_process():
            pwidgets = [progressbar.Percentage(), " ", progressbar.Counter(format='%(value)02d/%(max_value)d'), " ", progressbar.Bar(), " ",
                ...
                progressbar.Variable('sdl', width=1), ",", progressbar.Variable('adl', width=1), ",",
                progressbar.Variable('psnr', width=1)]
            pbar = progressbar.ProgressBar(widgets=pwidgets, max_value=len(self.val_loader), prefix="Val img:").start()
        ...
        for batch, inputs in enumerate(self.val_loader):
            ...
            anneal_ratio=self.get_cos_anneal_ratio(epoch + batch / len(self.val_loader))
            outputs = self.model("val", inputs, cos_anneal_ratio=anneal_ratio)
            ...
            auxi_depth = outputs["depth_stage0"].detach().cpu().numpy()
            ...
            # mesh.apply_transform(trans_mat.cpu().numpy())
            ...
            os.makedirs(os.path.join(self.base_exp_dir, 'val_auxi_depth'), exist_ok=True)
            ...
            self.save_depth(auxi_depth, os.path.join(self.base_exp_dir, 'val_auxi_depth', '{}_epoch{}.png'.format(file_name, epoch)))
            np.save(os.path.join(self.base_exp_dir, 'val_render_depth', '{}_epoch{}.npy'.format(file_name, epoch)), render_depth)
            np.save(os.path.join(self.base_exp_dir, 'val_sdf_depth', '{}_epoch{}.npy'.format(file_name, epoch)), sdf_depth)
            np.save(os.path.join(self.base_exp_dir, 'val_auxi_depth', '{}_epoch{}.npy'.format(file_name, epoch)), auxi_depth)
            ...
            auxi_depth = auxi_depth[::skip, ::skip]
            mask_ref = (depth_ref > 0)
            render_depth_loss = torch.tensor((np.abs(render_depth - depth_ref) * mask_ref).sum() / (mask_ref.sum() + 1e-8))
            sdf_depth_loss = torch.tensor((np.abs(sdf_depth - depth_ref) * mask_ref * (sdf_depth>0)).sum() / ((mask_ref * (sdf_depth>0)).sum() + 1e-8))
            auxi_depth_loss = torch.tensor((np.abs(auxi_depth - depth_ref) * mask_ref).sum() / (mask_ref.sum() + 1e-8))
            ...
            scalar_opts = {
                ...
                "auxi_depth_loss": auxi_depth_loss.to(self.device)
            }
    ...
    def finetune(self):
        ...
        for step in range(self.start_epoch, self.epochs):
            ...
            loss_res = self.loss(outputs, inputs, step, "finetune")
            loss = loss_res["loss"]
            ...
            if ((step + 1) % self.val_freq == 0) or ((step + 1) >= self.epochs):
                ...
                color_fine = val_outputs["color_fine"]
                ...
                psnr = 20.0 * torch.log10(1.0 / (((color_fine - val_inputs["color"].cpu())**2).mean()).sqrt())
                color_loss = F.l1_loss(color_fine, val_inputs["color"].cpu())
                
                print("psnr:",psnr.item(), "color_loss:", color_loss.item())
