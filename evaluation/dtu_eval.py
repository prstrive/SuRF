...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_dir', type=str, default='/data/pengr/cas_training/evaluation')
    ...
    for scan in scans:
        if args.mode == 'mesh':
            data = os.path.join(args.out_dir, "meshes", "final", "scan{}.ply".format(scan))
            ...
            # data = os.path.join(args.out_dir, "pcd", "scan{}.ply".format(scan))
            data = os.path.join(args.out_dir, "mvsnet{:0>3}_l3.ply".format(scan))
            pbar = tqdm(total=8)
    ...
    results["mean"] = {
        "d2s": mean(mean_d2s_list),
        's2d': mean(mean_s2d_list),
        'all': mean(over_all_list)
    }
    with open(args.out_dir + "/results.json", 'w') as fp:
