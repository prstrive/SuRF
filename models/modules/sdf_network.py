...
from .projector import lookup_volume, lookup_sparse_volume
...
class SDFNetworkSparse(nn.Module):
    def __init__(self, d_in, d_out, d_hidden, n_layers, skip_in=(4,), multires=0, bias=0.5, scale=1, geometric_init=True,
        ...
        super(SDFNetworkSparse, self).__init__()
    ...
    def forward(self, inputs, volumes, indexes):
        
        feats = lookup_sparse_volume(inputs.clone(), volumes, indexes)
        ...
        # return torch.cat([torch.tanh(x[:, :1]) * 3.5 / self.scale, x[:, 1:]], dim=-1)
        return torch.cat([x[:, :1] / self.scale, x[:, 1:]], dim=-1)

    def sdf(self, x, volumes, indexes):
        return self.forward(x, volumes, indexes)[:, :1]

    def sdf_hidden_appearance(self, x, volumes):
        return self.forward(x, volumes)
    
    @torch.enable_grad()
    def gradient(self, x, volumes, indexes):
        x.requires_grad_(True)
        y = self.sdf(x, volumes, indexes)
        d_output = torch.ones_like(y, requires_grad=False, device=y.device)
