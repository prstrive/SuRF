...
class TanksDataset(Dataset):
    def __init__(self, confs, mode):
        super(TanksDataset, self).__init__()
    ...
    def read_cam(self, filename):
        ...
        intrinsics_[0] *= self.img_hw[1] / 1920
        intrinsics_[1] *= self.img_hw[0] / 1080
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            img_path = os.path.join(self.data_dir, scan, 'images', '%08d.jpg' % vid)
            ...
            depths.append(depth)
            mask_path = os.path.join(self.data_dir, scan, 'masks', '%08d.jpg' % vid)
            if os.path.exists(mask_path):
                mask = (self.read_img(mask_path) / 255.0) > 0
            else:
