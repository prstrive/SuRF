...
def grid_sample_3d_sparse(sparse_volume, sparse_idx, pts):
    ...
    n_pts = pts.shape[0]
    bounding = torch.tensor([[-1, 1], [-1, 1], [-1, 1]]).type_as(pts)
    IW, IH, ID = sparse_idx.shape
    voxel_size = (bounding[:, 1] - bounding[:, 0]) / (torch.tensor([IW, IH, ID]).type_as(pts) - 1)
    coords = (pts - bounding[:, 0].unsqueeze(0)) / voxel_size.unsqueeze(0)
    ...
    index_volume = sparse_idx.reshape(-1)
    
    # back view
    bnw_idx = torch.gather(index_volume, 0,
                           (iz_bnw * ID ** 2 + iy_bnw * IW + ix_bnw).long().view(-1))
    bne_idx = torch.gather(index_volume, 0,
                           (iz_bne * ID ** 2 + iy_bne * IW + ix_bne).long().view(-1))
    bsw_idx = torch.gather(index_volume, 0,
                           (iz_bsw * ID ** 2 + iy_bsw * IW + ix_bsw).long().view(-1))
    bse_idx = torch.gather(index_volume, 0,
                           (iz_bse * ID ** 2 + iy_bse * IW + ix_bse).long().view(-1))

    # front view
    fnw_idx = torch.gather(index_volume, 0,
                           (iz_fnw * ID ** 2 + iy_fnw * IW + ix_fnw).long().view(-1))
    fne_idx = torch.gather(index_volume, 0,
                           (iz_fne * ID ** 2 + iy_fne * IW + ix_fne).long().view(-1))
    fsw_idx = torch.gather(index_volume, 0,
                           (iz_fsw * ID ** 2 + iy_fsw * IW + ix_fsw).long().view(-1))
    fse_idx = torch.gather(index_volume, 0,
                           (iz_fse * ID ** 2 + iy_fse * IW + ix_fse).long().view(-1))
    
    bnw_val = torch.zeros(n_pts, c).type_as(sparse_volume)
    bnw_val[bnw_idx!=-1] = torch.gather(sparse_volume, 0, bnw_idx[bnw_idx!=-1].long().view(-1, 1).repeat(1, c))
    bne_val = torch.zeros(n_pts, c).type_as(sparse_volume)
    bne_val[bne_idx!=-1] = torch.gather(sparse_volume, 0, bne_idx[bne_idx!=-1].long().view(-1, 1).repeat(1, c))
    bsw_val = torch.zeros(n_pts, c).type_as(sparse_volume)
    bsw_val[bsw_idx!=-1] = torch.gather(sparse_volume, 0, bsw_idx[bsw_idx!=-1].long().view(-1, 1).repeat(1, c))
    bse_val = torch.zeros(n_pts, c).type_as(sparse_volume)
    bse_val[bse_idx!=-1] = torch.gather(sparse_volume, 0, bse_idx[bse_idx!=-1].long().view(-1, 1).repeat(1, c))
    
    fnw_val = torch.zeros(n_pts, c).type_as(sparse_volume)
    fnw_val[fnw_idx!=-1] = torch.gather(sparse_volume, 0, fnw_idx[fnw_idx!=-1].long().view(-1, 1).repeat(1, c))
    fne_val = torch.zeros(n_pts, c).type_as(sparse_volume)
    fne_val[fne_idx!=-1] = torch.gather(sparse_volume, 0, fne_idx[fne_idx!=-1].long().view(-1, 1).repeat(1, c))
    fsw_val = torch.zeros(n_pts, c).type_as(sparse_volume)
    fsw_val[fsw_idx!=-1] = torch.gather(sparse_volume, 0, fsw_idx[fsw_idx!=-1].long().view(-1, 1).repeat(1, c))
    fse_val = torch.zeros(n_pts, c).type_as(sparse_volume)
    fse_val[fse_idx!=-1] = torch.gather(sparse_volume, 0, fse_idx[fse_idx!=-1].long().view(-1, 1).repeat(1, c))
    
    out_val = (
        # back
            bnw_val * bnw.unsqueeze(-1) +
            bne_val * bne.unsqueeze(-1) +
            bsw_val * bsw.unsqueeze(-1) +
            bse_val * bse.unsqueeze(-1) +
            # front
            fnw_val * fnw.unsqueeze(-1) +
            fne_val * fne.unsqueeze(-1) +
            fsw_val * fsw.unsqueeze(-1) +
            fse_val * fse.unsqueeze(-1)
...
def lookup_sparse_volume(pts, sparse_volumes, sparse_idxs):
    
    pts = pts.flip(dims=[-1])
    
    if isinstance(sparse_volumes, torch.Tensor):
        feat = grid_sample_3d_sparse(sparse_volumes, sparse_idxs, pts)
    else:
        feats = []
        for sparse_volume, sparse_idx in zip(sparse_volumes, sparse_idxs):
            feat = grid_sample_3d_sparse(sparse_volume, sparse_idx, pts)
            feats.append(feat)
...
def lookup_triplane(pts, triplanes, sample_mode="grad"):
    """
    pts: (n_pts, 3), default range from -1 to 1
    triplanes: list of map
    """
    n_pts, _ = pts.shape
    # x = pts.unsqueeze(0).unsqueeze(0).unsqueeze(0).flip(dims=[-1])
    ...
    for triplane in triplanes:
        xy = pts[..., :2]
        xz = torch.stack([pts[..., 0], pts[..., 2]], dim=-1)
        yz = torch.stack([pts[..., 1], pts[..., 2]], dim=-1)
        if sample_mode == "grad":
            feat_xy = grid_sample(triplane["xy"], xy)
            feat_xz = grid_sample(triplane["xz"], xz)
            feat_yz = grid_sample(triplane["yz"], yz)
            feat = feat_xy + feat_xz + feat_yz
        else:
            feat_xy = F.grid_sample(triplane["xy"], xy, mode=sample_mode)
            feat_xz =F.grid_sample(triplane["xz"], xz, mode=sample_mode)
            feat_yz = F.grid_sample(triplane["yz"], yz, mode=sample_mode)
            feat = feat_xy + feat_xz + feat_yz
            
        feat = feat.reshape(-1, n_pts).permute(1, 0)
        feats.append(feat)
    feat = torch.cat(feats, dim=-1)
...
def surface_patch_warp2(pts_sdf0, gradients_sdf0, images, intrinsics, poses, patch_size=11):
    """ 
    pts: (n_pts, 3)
    normal: (n_pts, 3)
    intrs: (n_view, 4, 4)
