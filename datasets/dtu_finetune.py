...
class DTUDatasetFinetune(Dataset):
    def __init__(self, confs, mode):
        super(DTUDatasetFinetune, self).__init__()
        ...
        self.intrs, self.c2ws, self.near_fars, self.scale_factor, self.scale_mat, self.w2c_ref, self.w2c_ref_inv = self.read_cam_info()
        ...
        self.w2c_ref_inv = torch.from_numpy(self.w2c_ref_inv.astype(np.float32))
        self.scale_mat = torch.from_numpy(self.scale_mat.astype(np.float32))
        
        # self.images_lis = [os.path.join(self.data_dir, self.scene, 'image', '%06d.png' % vid) for vid in self.all_views]
        # self.masks_lis = [os.path.join(self.data_dir, self.scene, 'mask', '%03d.png' % vid) for vid in self.all_views]
        
        self.images_lis = [os.path.join(self.data_dir, 'Rectified_raw/{}/rect_{:0>3}_3_r5000.png'.format(self.scene, vid + 1)) for vid in self.all_views]
        self.masks_lis = [os.path.join(self.data_dir, 'Depths_raw/{}/depth_visual_{:0>4}.png'.format(self.scene, vid)) for vid in self.all_views]
        ...
        pxyz = np.matmul(self.w2c_ref, np.concatenate([pxyz, np.ones_like(pxyz[..., :1])], axis=1)[..., None])[:, :3, 0]
        pxyz = torch.from_numpy(pxyz)
        self.pseudo_pts = (pxyz - self.scale_mat[:3, 3][None]) / self.scale_mat[0, 0]
        
        self.scale_mat = self.w2c_ref_inv @ self.scale_mat
    ...
    def read_cam_info(self):
        ...
        return new_intrs, c2ws, new_near_fars, scale_factor, scale_mat, w2c_ref, w2c_ref_inv
